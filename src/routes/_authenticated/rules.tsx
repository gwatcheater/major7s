    ...
    <>
      <style>{`
        ...
        @media (min-width: 1024px) {
          ...
          .rules-hero-title { color: #1a1a1a !important; }
          .rules-hero-sub { color: #999 !important; }
          .rules-hero-border { border-color: rgba(0,0,0,0.08) !important; }
          .rules-divider { background: rgba(0,0,0,0.07) !important; }
          .rules-section-body { color: #555 !important; }
          .rules-section-strong { color: #1a1a1a !important; }
          .rules-scoring-title { color: #9a7a2e !important; }
          .rules-card { background: #f5f5f3 !important; border-color: rgba(0,0,0,0.07) !important; }
          .rules-card-icon { background: #e8e8e6 !important; color: #666 !important; }
          .rules-card-title { color: #1a1a1a !important; }
          .rules-card-body { color: #888 !important; }
          .rules-owgr-body { color: #555 !important; }
          .rules-owgr-strong { color: #1a1a1a !important; }
          .rules-pill { color: #aaa !important; border-color: rgba(0,0,0,0.12) !important; }
          .rules-footer-border { border-color: rgba(0,0,0,0.07) !important; }
          .rules-nf-tag { color: #c0302f !important; }
        }
      `}</style>
      ...
      <div className="rules-page">
        <div className="max-w-2xl mx-auto px-6 py-10 pb-20">
          ...
          {/* Hero */}
          <div className="rules-hero-border pb-8 mb-8" style={{ borderBottom: "0.5px solid rgba(255,255,255,0.1)" }}>
            <h1
              className="rules-hero-title text-4xl font-medium leading-tight mb-2"
            ...
            <p className="rules-hero-sub text-sm italic" style={{ color: "rgba(255,255,255,0.35)" }}>
          ...
          {/* Scoring panel */}
          <div className="rounded-xl p-5 flex gap-4 items-start mb-2.5" style={{ background: "rgba(201,168,76,0.07)", border: "0.5px solid rgba(201,168,76,0.25)" }}>
            ...
            <div>
              <p className="rules-scoring-title text-sm font-medium mb-1" style={{ color: "var(--gold)" }}>Scoring</p>
              <p className="rules-section-body text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {["1st = 1 pt", "5th = 5 pts", "12th = 12 pts", "40th = 40 pts", "65th = 65 pts"].map((eg) => (
                  <span key={eg} className="text-[10px] font-medium px-2 py-0.5 rounded" style={{ background: "rgba(201,168,76,0.08)", border: "0.5px solid rgba(201,168,76,0.2)", color: "var(--gold)" }}>
                    {eg}
                  </span>
                ))}
              </div>
          ...
          {/* To win panel */}
          <div className="rounded-xl p-5 flex gap-4 items-start mb-2.5" style={{ background: "rgba(201,168,76,0.13)", border: "1.5px solid rgba(201,168,76,0.5)" }}>
            ...
            <div>
              <p className="rules-scoring-title text-sm font-medium mb-1" style={{ color: "var(--gold)" }}>To win</p>
              <p className="rules-section-body text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>
                <span className="rules-scoring-title font-medium" style={{ color: "var(--gold)" }}>Lowest total score wins.</span>{" "}
          ...
          {/* The CUT panel */}
          <div className="rounded-xl p-5 flex gap-4 items-start mb-2.5" style={{ background: "rgba(226,75,74,0.07)", border: "0.5px solid rgba(226,75,74,0.22)" }}>
            ...
            <div>
              ...
              <p className="rules-section-body text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>
                ...
                <span className="rules-section-strong font-medium" style={{ color: "rgba(255,255,255,0.65)" }}>Any pick who misses the CUT scores 100 pts.</span>{" "}
                With five of your seven picks counting, a cut hurts - but doesn't end your tournament.
              </p>
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                <span className="text-[11px] font-medium px-2.5 py-0.5 rounded" style={{ border: "0.5px solid rgba(226,75,74,0.5)", background: "rgba(226,75,74,0.14)", color: "#E24B4A" }}>
                  CUT = 100 pts
                </span>
              </div>
            </div>
          ...
          {/* Non-finishers panel */}
          <div className="rounded-xl p-5 flex gap-4 items-start" style={{ background: "rgba(239,159,39,0.07)", border: "0.5px solid rgba(239,159,39,0.22)" }}>
            ...
            <div>
              ...
              <p className="rules-section-body text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>
                ...
                <span className="rules-section-strong font-medium" style={{ color: "rgba(255,255,255,0.65)" }}>100 pts</span>{" "}
              ...
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {["WD - Withdrawal", "DQ - Disqualified", "DNF - Did not finish", "DNS - Did not start", "MDF - Made cut, did not finish"].map((tag) => (
                  <span key={tag} className="rules-nf-tag text-[10px] font-medium px-2 py-0.5 rounded" style={{ border: "0.5px solid rgba(226,75,74,0.35)", background: "rgba(226,75,74,0.1)", color: "rgba(226,75,74,0.8)" }}>
                    {tag}
                  </span>
                ))}
              </div>
          ...
          {/* Divider */}
          <div className="rules-divider my-7" style={{ height: "0.5px", background: "rgba(255,255,255,0.07)" }} />
          ...
          <div className="grid grid-cols-2 gap-2">
            {picks.map((card) => (
              <div key={card.title} className="rules-card rounded-lg p-4 flex gap-2.5 items-start" style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)" }}>
                <div className="rules-card-icon w-7 h-7 rounded-md flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" }}>
                  {card.icon}
                </div>
                <div>
                  <p className="rules-card-title text-[11px] font-medium mb-0.5" style={{ color: "rgba(255,255,255,0.6)" }}>{card.title}</p>
                  <p className="rules-card-body text-[11px] leading-relaxed" style={{ color: "rgba(255,255,255,0.3)" }}>{card.body}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rules-card rounded-xl p-5 flex gap-4 items-start" style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)" }}>
            <div className="rules-card-icon w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" }}>
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
            </div>
            <div className="rules-owgr-body text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>
              <p className="mb-3">
                Buckets are filled using the{" "}
                <span className="rules-owgr-strong font-medium" style={{ color: "rgba(255,255,255,0.6)" }}>Official World Golf Ranking (OWGR)</span>
          ...
          {/* Major pills footer */}
          <div className="rules-footer-border flex flex-wrap gap-1.5 justify-center mt-8 pt-6" style={{ borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
            {["Masters", "PGA Championship", "US Open", "The Open"].map((m) => (
              <span key={m} className="rules-pill text-[10px] px-3 py-1 rounded-full" style={{ color: "rgba(255,255,255,0.3)", border: "0.5px solid rgba(255,255,255,0.1)" }}>
                {m}
              </span>
            ))}
          </div>
