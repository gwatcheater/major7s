...
function RulesPage() {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--forest-deep)", color: "#fff" }}>
      <div className="max-w-2xl mx-auto px-6 py-10 pb-20">

        {/* Hero */}
        <div className="pb-8 mb-8" style={{ borderBottom: "0.5px solid rgba(255,255,255,0.08)" }}>
          <h1 className="text-4xl font-medium leading-tight mb-2" style={{ letterSpacing: "-0.02em", color: "#fff" }}>
            Four Majors.<br />
            Seven Picks.<br />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-2.5">
              {["1st = 1 pt", "5th = 5 pts", "12th = 12 pts", "40th = 40 pts", "65th = 65 pts"].map((eg) => (
                <span key={eg} className="text-[10px] font-medium px-2 py-0.5 rounded" style={{ background: "rgba(201,168,76,0.08)", border: "0.5px solid rgba(201,168,76,0.2)", color: "rgba(201,168,76,0.7)" }}>
                  {eg}
                </span>
              ))}
            </div>
        ...
        <div className="rounded-xl p-5 flex gap-4 items-start mb-2.5" style={{ background: "rgba(226,75,74,0.07)", border: "0.5px solid rgba(226,75,74,0.22)" }}>
          ...
          <div>
            ...
            <p className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.65)", fontWeight: 500 }}>Any pick who misses the CUT scores 100 pts.</span>{" "}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-2.5">
              {["WD - Withdrawal", "DQ - Disqualified", "DNF - Did not finish", "DNS - Did not start", "MDF - Made cut, did not finish"].map((tag) => (
                <span key={tag} className="text-[10px] font-medium px-2 py-0.5 rounded" style={{ border: "0.5px solid rgba(226,75,74,0.35)", background: "rgba(226,75,74,0.1)", color: "rgba(226,75,74,0.8)" }}>
                  {tag}
                </span>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {picks.map((card) => (
            <div key={card.title} className="rounded-lg p-4 flex gap-2.5 items-start" style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.07)" }}>
              <div className="w-7 h-7 rounded-md flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" }}>
                {card.icon}
              </div>
              <div>
                <p className="text-[11px] font-medium mb-0.5" style={{ color: "rgba(255,255,255,0.6)" }}>{card.title}</p>
                <p className="text-[11px] leading-relaxed" style={{ color: "rgba(255,255,255,0.3)" }}>{card.body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-xl p-5 flex gap-4 items-start" style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)" }}>
          <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" }}>
          ...
          <div className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>
            <p className="mb-3">
              ...
              <span style={{ color: "rgba(255,255,255,0.6)", fontWeight: 500 }}>Official World Golf Ranking (OWGR)</span>
        ...
        {/* Major pills footer */}
        <div className="flex flex-wrap gap-1.5 justify-center mt-8 pt-6" style={{ borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
          {["Masters", "PGA Championship", "US Open", "The Open"].map((m) => (
            <span key={m} className="text-[10px] px-3 py-1 rounded-full" style={{ color: "rgba(255,255,255,0.3)", border: "0.5px solid rgba(255,255,255,0.1)" }}>
              {m}
            </span>
          ))}
        </div>
